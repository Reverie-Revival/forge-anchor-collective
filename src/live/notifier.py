...
def alert_order_failed(stream_name: str, model_id: int, attempted_usd: float, error: str) -> None:
    ...
    once two models share one Kraken account: insufficient USD balance
    because both models' streams signaled close together. Previously this
    only logged an error with no alert."""
    _dispatch(
