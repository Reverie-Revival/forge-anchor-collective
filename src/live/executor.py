...
def _ensure_market_data_fresh(conn, now: datetime, closed_tfs: set, model_id: int) -> None:
    ...
    Separate from _preflight_check below, which only catches gross
    (hours-scale) staleness -- this catches the much tighter race where
    market_data_updater simply hasn't landed this cycle's bar yet."""
    if not closed_tfs:
        return
