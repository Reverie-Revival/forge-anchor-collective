...
**Update, same day, later session: the deeper gap flagged above (live code never captured Kraken's real per-trade fee/fill price) is now fixed too — see "Real per-trade fee capture" entry at the top of "Done This Session" below.** That work also surfaced and fixed a second, independent bug: `live-model-3` had never received this section's fee-constant correction at all (still running 0.25%/0.40% in production until this session).
...
## Done This Session (2026-08-03, latest) — Real per-trade fee capture (the deeper gap from earlier today), a second stale-branch fee bug found along the way, and a backtester/scripts cleanup pass
...
**Second bug found while cherry-picking to `live-model-3`: that branch had never received the original 0.25%/0.40% → 0.40%/0.80% fee-tier correction from earlier today at all**, nor `kraken_client.get_fee_tier()`, nor `src/live/fee_check.py`, nor the fee-drift healthcheck wiring — none of it ever made it onto that branch. Root cause: the "one shared Kraken account, one shared fee-drift check suffices" assumption from earlier today's fee-drift-safeguard work doesn't actually hold, because Model 1 and Model 3 live on **separate branches**, each keeping its **own independent copy** of `MAKER_FEE`/`TAKER_FEE` — the two copies can (and did) drift apart from *each other*, not just from Kraken's real rate. No real exit had happened yet on `live-model-3` under the wrong rate (position still `OPEN`, entry-only), so nothing realized was corrupted by this either. Fixed: corrected the constants, ported `get_fee_tier()` + `fee_check.py`, and wired `check_fee_drift()` into **both** `healthcheck.py` and `blended_healthcheck.py` on `main`, `live-model-1`, and `live-model-3` — the old "check once" comments in `fee_check.py`/`healthcheck.py` were corrected to explain why both are needed now.
...
**Deployed:** `main` (`5529d34`, `be1641f`, `f980eea`), `live-model-1` (`b2c26b8` — targeted cherry-pick, kept that branch's own local fee-constant copy, ported the new `_fake_kraken.py`/`conftest.py`/test file so this critical fix has real test coverage on that branch too), `live-model-3` (`56977c6` fee-constant + safeguard fix, `ed3a392` fee-capture cherry-pick — full 27/27 `tests/live/` pass). Supabase migration applied and confirmed against the two real currently-open positions (Model 1 `lot_id=2`, Model 3 `position_id=1` — both correctly `NULL` on the new legacy columns, `fee_is_estimated` defaults `false` until their next exit triggers the fallback path for real).
...
**⚠️ Correction (2026-08-03, latest session): that "one shared account, one check" assumption was wrong, and it's exactly how the bug below happened.** Model 1 and Model 3 live on separate branches, each with their OWN independent copy of `MAKER_FEE`/`TAKER_FEE` — those copies can drift from *each other*, not just from Kraken's real rate, and `live-model-3`'s did (see the real-fee-capture session entry above for the full story). Fixed: `check_fee_drift()` now also runs from `blended_healthcheck.py`, on `main`, `live-model-1`, and `live-model-3` alike. Comments in `fee_check.py`/`healthcheck.py` updated to explain why both are needed, not just one.
...
Live code never captured Kraken's real per-trade fee/fill-price, only estimated via constants. Fixed — see the "Real per-trade fee capture" entry at the top of "Done This Session" above for the full story, including the mixed-legacy-fee bug found during re-verification and the second stale-constant bug found on `live-model-3` along the way.
...
| `live-model-3` (branch-level) | Never received the 2026-08-03 fee-constant correction (0.25%/0.40% → 0.40%/0.80%) or the fee-drift safeguard at all — diverged silently from `main`/`live-model-1` | Corrected constants, ported `get_fee_tier()`/`fee_check.py`, wired `check_fee_drift()` into `blended_healthcheck.py` |
